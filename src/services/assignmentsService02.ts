...
export interface AssignmentsItem02 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: AssignmentsItem02[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "assignments-02-" + String(i).padStart(4, "0"),
...
export async function fetchAssignmentsList02(params?: ListQueryParams): Promise<PaginatedResponse<AssignmentsItem02>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createAssignments02(input: Partial<AssignmentsItem02>): Promise<AssignmentsItem02> {
  if (USE_MOCK) {
    ensure();
    const item: AssignmentsItem02 = {
      id: "assignments-02-" + crypto.randomUUID().slice(0, 8),
