...
export interface AssignmentsItem01 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: AssignmentsItem01[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "assignments-01-" + String(i).padStart(4, "0"),
...
export async function fetchAssignmentsList01(params?: ListQueryParams): Promise<PaginatedResponse<AssignmentsItem01>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createAssignments01(input: Partial<AssignmentsItem01>): Promise<AssignmentsItem01> {
  if (USE_MOCK) {
    ensure();
    const item: AssignmentsItem01 = {
      id: "assignments-01-" + crypto.randomUUID().slice(0, 8),
