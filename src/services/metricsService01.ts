...
export interface MetricsItem01 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: MetricsItem01[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "metrics-01-" + String(i).padStart(4, "0"),
      name: "Metrics item " + i,
...
export async function fetchMetricsList01(params?: ListQueryParams): Promise<PaginatedResponse<MetricsItem01>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createMetrics01(input: Partial<MetricsItem01>): Promise<MetricsItem01> {
  if (USE_MOCK) {
    ensure();
    const item: MetricsItem01 = {
      id: "metrics-01-" + crypto.randomUUID().slice(0, 8),
