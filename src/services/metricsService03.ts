...
export interface MetricsItem03 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: MetricsItem03[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "metrics-03-" + String(i).padStart(4, "0"),
      name: "Metrics item " + i,
...
export async function fetchMetricsList03(params?: ListQueryParams): Promise<PaginatedResponse<MetricsItem03>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createMetrics03(input: Partial<MetricsItem03>): Promise<MetricsItem03> {
  if (USE_MOCK) {
    ensure();
    const item: MetricsItem03 = {
      id: "metrics-03-" + crypto.randomUUID().slice(0, 8),
