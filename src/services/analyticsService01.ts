...
export interface AnalyticsItem01 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: AnalyticsItem01[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "analytics-01-" + String(i).padStart(4, "0"),
...
export async function fetchAnalyticsList01(params?: ListQueryParams): Promise<PaginatedResponse<AnalyticsItem01>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createAnalytics01(input: Partial<AnalyticsItem01>): Promise<AnalyticsItem01> {
  if (USE_MOCK) {
    ensure();
    const item: AnalyticsItem01 = {
      id: "analytics-01-" + crypto.randomUUID().slice(0, 8),
