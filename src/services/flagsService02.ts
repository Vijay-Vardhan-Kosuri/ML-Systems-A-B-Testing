...
const USE_MOCK = true;
const store: FlagsItem02[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "flags-02-" + String(i).padStart(4, "0"),
      name: "Flags item " + i,
...
export async function fetchFlagsList02(params?: ListQueryParams): Promise<PaginatedResponse<FlagsItem02>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createFlags02(input: Partial<FlagsItem02>): Promise<FlagsItem02> {
  if (USE_MOCK) {
    ensure();
    const item: FlagsItem02 = {
      id: "flags-02-" + crypto.randomUUID().slice(0, 8),
