...
const USE_MOCK = true;
const store: FlagsItem03[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "flags-03-" + String(i).padStart(4, "0"),
      name: "Flags item " + i,
...
export async function fetchFlagsList03(params?: ListQueryParams): Promise<PaginatedResponse<FlagsItem03>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createFlags03(input: Partial<FlagsItem03>): Promise<FlagsItem03> {
  if (USE_MOCK) {
    ensure();
    const item: FlagsItem03 = {
      id: "flags-03-" + crypto.randomUUID().slice(0, 8),
