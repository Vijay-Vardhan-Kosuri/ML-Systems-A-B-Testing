...
export interface AnalyticsItem02 {
  id: string;
  name: string;
  status: string;
  experimentId?: string;
...
const USE_MOCK = true;
const store: AnalyticsItem02[] = [];

function ensure(): void {
  if (store.length) return;
  for (let i = 1; i <= 25; i++) {
    store.push({
      id: "analytics-02-" + String(i).padStart(4, "0"),
...
export async function fetchAnalyticsList02(params?: ListQueryParams): Promise<PaginatedResponse<AnalyticsItem02>> {
  if (USE_MOCK) {
    ensure();
    let data = [...store];
    if (params?.search) {
...
export async function createAnalytics02(input: Partial<AnalyticsItem02>): Promise<AnalyticsItem02> {
  if (USE_MOCK) {
    ensure();
    const item: AnalyticsItem02 = {
      id: "analytics-02-" + crypto.randomUUID().slice(0, 8),
