/**
 * recommendations experiment guardrails for ML A/B testing platform.
 * Production logic for engagement optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type RecommendationsGuardrailsConfig = {
...
export const defaultRecommendationsGuardrailsConfig: RecommendationsGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validateRecommendationsExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultRecommendationsGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateRecommendationsDurationDays(
...
export function scoreRecommendationsResults(results: ExperimentResult[]): {
...
export function buildRecommendationsSummary(
  ...
  const scored = scoreRecommendationsResults(results);
  return {
    experimentId: exp.id,
...
export function isRecommendationsReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendRecommendationsTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterRecommendationsByTag(
