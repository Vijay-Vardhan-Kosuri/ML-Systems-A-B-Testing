/**
 * onboarding experiment guardrails for ML A/B testing platform.
 * Production logic for retention optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type OnboardingGuardrailsConfig = {
...
export const defaultOnboardingGuardrailsConfig: OnboardingGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validateOnboardingExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultOnboardingGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateOnboardingDurationDays(
...
export function scoreOnboardingResults(results: ExperimentResult[]): {
...
export function buildOnboardingSummary(
  ...
  const scored = scoreOnboardingResults(results);
  return {
    experimentId: exp.id,
    domain: "onboarding",
    metric: "retention",
...
export function isOnboardingReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendOnboardingTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterOnboardingByTag(
