/**
 * checkout experiment guardrails for ML A/B testing platform.
 * Production logic for conversion optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type CheckoutGuardrailsConfig = {
...
export const defaultCheckoutGuardrailsConfig: CheckoutGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validateCheckoutExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultCheckoutGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateCheckoutDurationDays(
...
export function scoreCheckoutResults(results: ExperimentResult[]): {
...
export function buildCheckoutSummary(
  ...
  const scored = scoreCheckoutResults(results);
  return {
    experimentId: exp.id,
    domain: "checkout",
    metric: "conversion",
...
export function isCheckoutReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendCheckoutTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterCheckoutByTag(
