/**
 * pricing experiment guardrails for ML A/B testing platform.
 * Production logic for revenue optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type PricingGuardrailsConfig = {
...
export const defaultPricingGuardrailsConfig: PricingGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validatePricingExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultPricingGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimatePricingDurationDays(
...
export function scorePricingResults(results: ExperimentResult[]): {
...
export function buildPricingSummary(
  ...
  const scored = scorePricingResults(results);
  return {
    experimentId: exp.id,
    domain: "pricing",
    metric: "revenue",
...
export function isPricingReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendPricingTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterPricingByTag(
