/**
 * search experiment guardrails for ML A/B testing platform.
 * Production logic for click_through optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type SearchGuardrailsConfig = {
...
export const defaultSearchGuardrailsConfig: SearchGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validateSearchExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultSearchGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateSearchDurationDays(
...
export function scoreSearchResults(results: ExperimentResult[]): {
...
export function isSearchReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendSearchTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterSearchByTag(
