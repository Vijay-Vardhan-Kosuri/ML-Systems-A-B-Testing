/**
 * ml_ranker experiment guardrails for ML A/B testing platform.
 * Production logic for ndcg optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type MlRankerGuardrailsConfig = {
...
export const defaultMlRankerGuardrailsConfig: MlRankerGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
  primaryMetric: "ndcg",
...
export function validateMlRankerExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultMlRankerGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateMlRankerDurationDays(
...
export function scoreMlRankerResults(results: ExperimentResult[]): {
...
export function buildMlRankerSummary(
  exp: Experiment,
  results: ExperimentResult[],
): Record<string, unknown> {
  const scored = scoreMlRankerResults(results);
  return {
    experimentId: exp.id,
    domain: "ml_ranker",
    metric: "ndcg",
...
export function isMlRankerReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendMlRankerTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterMlRankerByTag(
