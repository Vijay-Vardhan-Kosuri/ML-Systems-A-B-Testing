/**
 * mobile_nav experiment guardrails for ML A/B testing platform.
 * Production logic for session_depth optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type MobileNavGuardrailsConfig = {
...
export const defaultMobileNavGuardrailsConfig: MobileNavGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
  primaryMetric: "session_depth",
  significanceLevel: 0.05,
  trafficCapPercent: 50,
};

export function validateMobileNavExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultMobileNavGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateMobileNavDurationDays(
...
export function scoreMobileNavResults(results: ExperimentResult[]): {
...
export function buildMobileNavSummary(
  exp: Experiment,
  results: ExperimentResult[],
): Record<string, unknown> {
  const scored = scoreMobileNavResults(results);
  return {
    experimentId: exp.id,
    domain: "mobile_nav",
...
export function isMobileNavReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendMobileNavTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterMobileNavByTag(
