/**
 * email experiment guardrails for ML A/B testing platform.
 * Production logic for open_rate optimization experiments.
 */
import type { Experiment, Variant, ExperimentResult } from "@/shared/types";

export type EmailGuardrailsConfig = {
...
export const defaultEmailGuardrailsConfig: EmailGuardrailsConfig = {
  minSampleSize: 5000,
  maxDurationDays: 28,
...
export function validateEmailExperiment(
  exp: Partial<Experiment>,
  variants: Variant[],
  config = defaultEmailGuardrailsConfig,
): string[] {
  const errors: string[] = [];
...
export function estimateEmailDurationDays(
...
export function scoreEmailResults(results: ExperimentResult[]): {
...
export function buildEmailSummary(
  ...
  const scored = scoreEmailResults(results);
  return {
    experimentId: exp.id,
    domain: "email",
    metric: "open_rate",
...
export function isEmailReadyToShip(
  exp: Experiment,
  results: ExperimentResult[],
...
export function recommendEmailTraffic(
  currentDaily: number,
  targetSample: number,
  maxDays: number,
): number {
...
export function filterEmailByTag(
