...
/**
 * Experiment domain helpers 050
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive050(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable050(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart050(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop050(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus050(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments050(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc050(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName050(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus050(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus050(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment050(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants050(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays050(
...
export function assignmentCountByVariant050(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus050(status: ExperimentStatus): string {
...
export function statusColor050(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate050<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment050(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
