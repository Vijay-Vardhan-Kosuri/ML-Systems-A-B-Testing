...
/**
 * Experiment domain helpers 026
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive026(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable026(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart026(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop026(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus026(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments026(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc026(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName026(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus026(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus026(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment026(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants026(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays026(
...
export function assignmentCountByVariant026(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus026(status: ExperimentStatus): string {
...
export function statusColor026(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate026<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment026(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
