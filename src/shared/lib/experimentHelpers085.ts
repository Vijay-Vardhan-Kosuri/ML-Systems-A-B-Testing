...
export function isActive085(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable085(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart085(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus085(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments085(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc085(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus085(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment085(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants085(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate085<T>(items: T[], page: number, size: number): T[] {
