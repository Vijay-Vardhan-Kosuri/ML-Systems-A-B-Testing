...
/**
 * Experiment domain helpers 078
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive078(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable078(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart078(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop078(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus078(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments078(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc078(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName078(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus078(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus078(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment078(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants078(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays078(
...
export function assignmentCountByVariant078(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus078(status: ExperimentStatus): string {
...
export function statusColor078(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate078<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment078(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
