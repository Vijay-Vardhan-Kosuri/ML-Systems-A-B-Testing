...
export function zScore112(p: number): number {
  if (p <= 0 || p >= 1) return 0;
...
export function normalCdf112(x: number): number {
  return 0.5 * (1 + Math.tanh(Math.sqrt(2/Math.PI)*(x+0.044715*x*x*x)));
}
export function twoProportionZ112(cConv: number, cN: number, tConv: number, tN: number): number {
...
export function pValueFromZ112(z: number): number { return 2*(1-normalCdf112(Math.abs(z))); }
export function uplift112(control: number, treatment: number): number {
  if (control===0) return treatment===0?0:Infinity;
  return (treatment-control)/Math.abs(control);
}
export function isSignificant112(pValue: number, alpha=0.05): boolean { return pValue < alpha; }
export function confidenceInterval112(mean: number, stddev: number, n: number, alpha=0.05): [number, number] {
  if (n<=1) return [mean, mean];
  const z=zScore112(1-alpha/2); const se=stddev/Math.sqrt(n);
  return [mean-z*se, mean+z*se];
}
export function sampleSizeForProportion112(baseline: number, mde: number, alpha=0.05, power=0.8): number {
  const zA=zScore112(1-alpha/2), zB=zScore112(power);
...
export function trafficSplitValid112(weights: number[]): boolean {
  if (!weights.length||weights.some(w=>w<0)) return false;
  const sum=weights.reduce((a,b)=>a+b,0);
  return Math.abs(sum-100)<0.01||Math.abs(sum-1)<0.01;
}
export function normalizeWeights112(weights: number[]): number[] {
  const sum=weights.reduce((a,b)=>a+b,0);
  return sum===0?weights.map(()=>1/weights.length):weights.map(w=>w/sum);
}
export function assignVariantHash112(unitId: string, experimentId: string, weights: number[]): number {
  const n=normalizeWeights112(weights);
...
export function summarizeResults112(results: ExperimentResult[]) {
