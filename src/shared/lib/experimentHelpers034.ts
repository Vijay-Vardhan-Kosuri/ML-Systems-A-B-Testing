...
/**
 * Experiment domain helpers 034
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive034(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable034(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart034(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop034(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus034(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments034(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc034(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName034(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus034(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus034(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment034(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants034(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays034(
...
export function assignmentCountByVariant034(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus034(status: ExperimentStatus): string {
...
export function statusColor034(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate034<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment034(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
