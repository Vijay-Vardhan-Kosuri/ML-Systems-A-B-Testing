...
/**
 * Experiment domain helpers 070
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive070(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable070(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart070(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop070(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus070(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments070(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc070(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName070(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus070(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus070(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment070(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants070(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays070(
...
export function assignmentCountByVariant070(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus070(status: ExperimentStatus): string {
...
export function statusColor070(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate070<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment070(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
