...
/**
 * Experiment domain helpers 068
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive068(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable068(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart068(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop068(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus068(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments068(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc068(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName068(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus068(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus068(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment068(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants068(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays068(
...
export function assignmentCountByVariant068(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus068(status: ExperimentStatus): string {
...
export function statusColor068(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate068<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment068(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
