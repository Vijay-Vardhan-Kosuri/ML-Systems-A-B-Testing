...
export function isActive102(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable102(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart102(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus102(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments102(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc102(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus102(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment102(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants102(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate102<T>(items: T[], page: number, size: number): T[] {
