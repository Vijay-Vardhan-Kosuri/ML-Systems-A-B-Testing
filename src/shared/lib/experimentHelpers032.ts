...
/**
 * Experiment domain helpers 032
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive032(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable032(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart032(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop032(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus032(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments032(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc032(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName032(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus032(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus032(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment032(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants032(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays032(
...
export function assignmentCountByVariant032(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus032(status: ExperimentStatus): string {
...
export function statusColor032(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate032<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment032(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
