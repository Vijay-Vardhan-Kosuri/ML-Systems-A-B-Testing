...
/**
 * Experiment domain helpers 044
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive044(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable044(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart044(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop044(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus044(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments044(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc044(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName044(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus044(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus044(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment044(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants044(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays044(
...
export function assignmentCountByVariant044(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus044(status: ExperimentStatus): string {
...
export function statusColor044(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate044<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment044(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
