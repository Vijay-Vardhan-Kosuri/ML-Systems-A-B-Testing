...
/**
 * Experiment domain helpers 024
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive024(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable024(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart024(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop024(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus024(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments024(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc024(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName024(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus024(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus024(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment024(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants024(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays024(
...
export function assignmentCountByVariant024(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus024(status: ExperimentStatus): string {
...
export function statusColor024(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate024<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment024(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
