...
export function isActive094(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable094(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart094(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus094(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments094(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc094(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus094(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment094(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants094(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate094<T>(items: T[], page: number, size: number): T[] {
