...
export function isActive090(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable090(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart090(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus090(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments090(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc090(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus090(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment090(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants090(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate090<T>(items: T[], page: number, size: number): T[] {
