...
/**
 * Experiment domain helpers 011
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive011(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable011(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart011(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop011(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus011(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments011(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc011(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName011(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus011(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus011(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment011(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants011(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays011(
...
export function assignmentCountByVariant011(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus011(status: ExperimentStatus): string {
...
export function statusColor011(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate011<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment011(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
