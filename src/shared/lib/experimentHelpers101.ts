...
export function isActive101(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable101(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart101(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus101(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments101(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc101(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus101(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment101(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants101(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate101<T>(items: T[], page: number, size: number): T[] {
