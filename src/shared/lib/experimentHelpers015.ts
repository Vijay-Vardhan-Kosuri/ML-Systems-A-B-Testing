...
/**
 * Experiment domain helpers 015
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive015(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable015(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart015(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop015(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus015(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments015(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc015(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName015(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus015(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus015(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment015(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants015(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays015(
...
export function assignmentCountByVariant015(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus015(status: ExperimentStatus): string {
...
export function statusColor015(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate015<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment015(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
