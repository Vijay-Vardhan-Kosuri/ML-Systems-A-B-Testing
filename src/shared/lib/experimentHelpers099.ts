...
export function isActive099(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable099(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart099(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus099(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments099(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc099(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus099(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment099(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants099(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate099<T>(items: T[], page: number, size: number): T[] {
