...
/**
 * Experiment domain helpers 022
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive022(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable022(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart022(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop022(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus022(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments022(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc022(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName022(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus022(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus022(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment022(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants022(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays022(
...
export function assignmentCountByVariant022(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus022(status: ExperimentStatus): string {
...
export function statusColor022(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate022<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment022(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
