...
/**
 * Experiment domain helpers 054
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive054(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable054(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart054(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop054(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus054(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments054(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc054(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName054(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus054(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus054(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment054(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants054(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays054(
...
export function assignmentCountByVariant054(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus054(status: ExperimentStatus): string {
...
export function statusColor054(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate054<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment054(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
