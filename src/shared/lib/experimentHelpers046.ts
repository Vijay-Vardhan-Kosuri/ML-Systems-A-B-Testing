...
/**
 * Experiment domain helpers 046
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive046(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable046(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart046(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop046(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus046(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments046(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc046(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName046(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus046(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus046(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment046(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants046(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays046(
...
export function assignmentCountByVariant046(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus046(status: ExperimentStatus): string {
...
export function statusColor046(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate046<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment046(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
