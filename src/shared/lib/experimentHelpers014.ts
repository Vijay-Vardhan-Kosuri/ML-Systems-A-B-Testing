...
/**
 * Experiment domain helpers 014
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive014(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable014(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart014(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop014(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus014(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments014(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc014(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName014(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus014(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus014(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment014(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants014(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays014(
...
export function assignmentCountByVariant014(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus014(status: ExperimentStatus): string {
...
export function statusColor014(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate014<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment014(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
