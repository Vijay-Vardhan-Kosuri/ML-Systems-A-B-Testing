...
/**
 * Experiment domain helpers 029
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive029(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable029(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart029(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop029(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus029(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments029(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc029(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName029(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus029(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus029(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment029(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants029(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays029(
...
export function assignmentCountByVariant029(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus029(status: ExperimentStatus): string {
...
export function statusColor029(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate029<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment029(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
