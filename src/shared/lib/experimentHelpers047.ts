...
/**
 * Experiment domain helpers 047
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive047(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable047(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart047(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop047(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus047(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments047(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc047(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName047(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus047(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus047(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment047(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants047(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays047(
...
export function assignmentCountByVariant047(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus047(status: ExperimentStatus): string {
...
export function statusColor047(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate047<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment047(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
