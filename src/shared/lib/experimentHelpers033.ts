...
/**
 * Experiment domain helpers 033
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive033(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable033(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart033(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop033(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus033(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments033(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc033(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName033(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus033(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus033(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment033(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants033(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays033(
...
export function assignmentCountByVariant033(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus033(status: ExperimentStatus): string {
...
export function statusColor033(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate033<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment033(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
