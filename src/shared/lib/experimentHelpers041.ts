...
/**
 * Experiment domain helpers 041
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive041(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable041(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart041(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop041(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus041(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments041(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc041(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName041(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus041(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus041(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment041(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants041(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays041(
...
export function assignmentCountByVariant041(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus041(status: ExperimentStatus): string {
...
export function statusColor041(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate041<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment041(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
