...
/**
 * Experiment domain helpers 002
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive002(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable002(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart002(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop002(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus002(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments002(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc002(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName002(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus002(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus002(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment002(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants002(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays002(
...
export function assignmentCountByVariant002(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus002(status: ExperimentStatus): string {
...
export function statusColor002(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate002<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment002(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
