...
export function isActive105(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable105(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart105(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus105(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments105(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc105(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus105(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment105(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants105(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate105<T>(items: T[], page: number, size: number): T[] {
