...
/**
 * Experiment domain helpers 060
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive060(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable060(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart060(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop060(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus060(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments060(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc060(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName060(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus060(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus060(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment060(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants060(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays060(
...
export function assignmentCountByVariant060(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus060(status: ExperimentStatus): string {
...
export function statusColor060(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate060<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment060(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
