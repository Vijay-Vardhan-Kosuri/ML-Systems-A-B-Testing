...
export function isActive091(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable091(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart091(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus091(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments091(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc091(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus091(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment091(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants091(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate091<T>(items: T[], page: number, size: number): T[] {
