...
export function isActive086(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable086(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart086(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus086(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments086(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc086(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus086(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment086(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants086(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate086<T>(items: T[], page: number, size: number): T[] {
