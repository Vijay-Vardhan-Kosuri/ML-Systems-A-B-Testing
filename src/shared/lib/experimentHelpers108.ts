...
export function isActive108(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable108(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart108(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus108(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments108(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc108(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus108(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment108(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants108(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate108<T>(items: T[], page: number, size: number): T[] {
