...
/**
 * Experiment domain helpers 077
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive077(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable077(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart077(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop077(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus077(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments077(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc077(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName077(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus077(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus077(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment077(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants077(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays077(
...
export function assignmentCountByVariant077(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus077(status: ExperimentStatus): string {
...
export function statusColor077(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate077<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment077(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
