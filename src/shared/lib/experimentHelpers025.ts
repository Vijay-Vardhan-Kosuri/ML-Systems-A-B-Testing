...
/**
 * Experiment domain helpers 025
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive025(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable025(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart025(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop025(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus025(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments025(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc025(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName025(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus025(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus025(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment025(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants025(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays025(
...
export function assignmentCountByVariant025(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus025(status: ExperimentStatus): string {
...
export function statusColor025(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate025<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment025(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
