...
/**
 * Experiment domain helpers 018
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive018(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable018(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart018(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop018(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus018(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments018(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc018(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName018(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus018(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus018(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment018(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants018(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays018(
...
export function assignmentCountByVariant018(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus018(status: ExperimentStatus): string {
...
export function statusColor018(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate018<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment018(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
