...
/**
 * Experiment domain helpers 010
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive010(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable010(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart010(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop010(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus010(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments010(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc010(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName010(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus010(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus010(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment010(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants010(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays010(
...
export function assignmentCountByVariant010(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus010(status: ExperimentStatus): string {
...
export function statusColor010(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate010<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment010(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
