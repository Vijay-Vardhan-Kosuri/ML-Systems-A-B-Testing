...
/**
 * Experiment domain helpers 042
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive042(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable042(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart042(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop042(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus042(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments042(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc042(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName042(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus042(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus042(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment042(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants042(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays042(
...
export function assignmentCountByVariant042(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus042(status: ExperimentStatus): string {
...
export function statusColor042(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate042<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment042(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
