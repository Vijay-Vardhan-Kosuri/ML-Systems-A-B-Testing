...
export function isActive093(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable093(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart093(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus093(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments093(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc093(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus093(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment093(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants093(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate093<T>(items: T[], page: number, size: number): T[] {
