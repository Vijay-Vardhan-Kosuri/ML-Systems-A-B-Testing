...
/**
 * Experiment domain helpers 016
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive016(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable016(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart016(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop016(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus016(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments016(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc016(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName016(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus016(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus016(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment016(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants016(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays016(
...
export function assignmentCountByVariant016(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus016(status: ExperimentStatus): string {
...
export function statusColor016(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate016<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment016(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
