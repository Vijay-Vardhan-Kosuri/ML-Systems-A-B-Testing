...
export function isActive098(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable098(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart098(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus098(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments098(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc098(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus098(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment098(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants098(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate098<T>(items: T[], page: number, size: number): T[] {
