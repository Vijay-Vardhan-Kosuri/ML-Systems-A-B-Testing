...
/**
 * Experiment domain helpers 065
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive065(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable065(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart065(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop065(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus065(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments065(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc065(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName065(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus065(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus065(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment065(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants065(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays065(
...
export function assignmentCountByVariant065(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus065(status: ExperimentStatus): string {
...
export function statusColor065(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate065<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment065(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
