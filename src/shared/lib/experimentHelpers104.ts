...
export function isActive104(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable104(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart104(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus104(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments104(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc104(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus104(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment104(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants104(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate104<T>(items: T[], page: number, size: number): T[] {
