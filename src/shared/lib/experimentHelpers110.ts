...
export function isActive110(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable110(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart110(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus110(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments110(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc110(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus110(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment110(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants110(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate110<T>(items: T[], page: number, size: number): T[] {
