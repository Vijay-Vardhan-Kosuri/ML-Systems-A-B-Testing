...
/**
 * Experiment domain helpers 057
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive057(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable057(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart057(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop057(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus057(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments057(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc057(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName057(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus057(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus057(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment057(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants057(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays057(
...
export function assignmentCountByVariant057(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus057(status: ExperimentStatus): string {
...
export function statusColor057(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate057<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment057(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
