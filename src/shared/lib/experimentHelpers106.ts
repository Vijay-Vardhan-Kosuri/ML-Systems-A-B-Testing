...
export function isActive106(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable106(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart106(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus106(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments106(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc106(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus106(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment106(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants106(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate106<T>(items: T[], page: number, size: number): T[] {
