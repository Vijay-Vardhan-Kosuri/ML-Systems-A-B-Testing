...
export function isActive107(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable107(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart107(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus107(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments107(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc107(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus107(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment107(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants107(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate107<T>(items: T[], page: number, size: number): T[] {
