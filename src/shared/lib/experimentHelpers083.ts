...
export function isActive083(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable083(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart083(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus083(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments083(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc083(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus083(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment083(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants083(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate083<T>(items: T[], page: number, size: number): T[] {
