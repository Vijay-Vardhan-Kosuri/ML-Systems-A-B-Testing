...
/**
 * Experiment domain helpers 004
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive004(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable004(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart004(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop004(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus004(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments004(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc004(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName004(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus004(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus004(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment004(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants004(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays004(
...
export function assignmentCountByVariant004(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus004(status: ExperimentStatus): string {
...
export function statusColor004(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate004<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment004(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
