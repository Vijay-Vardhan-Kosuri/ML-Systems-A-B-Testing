...
/**
 * Experiment domain helpers 006
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive006(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable006(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart006(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop006(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus006(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments006(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc006(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName006(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus006(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus006(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment006(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants006(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays006(
...
export function assignmentCountByVariant006(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus006(status: ExperimentStatus): string {
...
export function statusColor006(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate006<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment006(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
