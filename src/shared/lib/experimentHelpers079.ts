...
/**
 * Experiment domain helpers 079
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive079(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable079(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart079(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop079(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus079(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments079(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc079(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName079(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus079(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus079(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment079(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants079(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays079(
...
export function assignmentCountByVariant079(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus079(status: ExperimentStatus): string {
...
export function statusColor079(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate079<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment079(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
