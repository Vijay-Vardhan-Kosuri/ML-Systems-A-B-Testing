...
export function isActive103(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable103(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart103(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus103(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments103(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc103(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus103(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment103(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants103(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate103<T>(items: T[], page: number, size: number): T[] {
