...
/**
 * Experiment domain helpers 080
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive080(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable080(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart080(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop080(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus080(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments080(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc080(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName080(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus080(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus080(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment080(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants080(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays080(
...
export function assignmentCountByVariant080(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus080(status: ExperimentStatus): string {
...
export function statusColor080(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate080<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment080(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
