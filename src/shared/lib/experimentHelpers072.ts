...
/**
 * Experiment domain helpers 072
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive072(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable072(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart072(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop072(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus072(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments072(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc072(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName072(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus072(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus072(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment072(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants072(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays072(
...
export function assignmentCountByVariant072(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus072(status: ExperimentStatus): string {
...
export function statusColor072(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate072<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment072(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
