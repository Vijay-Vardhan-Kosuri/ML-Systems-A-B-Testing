...
export function isActive096(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable096(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart096(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus096(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments096(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc096(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus096(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment096(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants096(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate096<T>(items: T[], page: number, size: number): T[] {
