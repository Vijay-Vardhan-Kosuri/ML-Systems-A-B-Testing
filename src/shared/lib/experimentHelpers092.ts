...
export function isActive092(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable092(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart092(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus092(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments092(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc092(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus092(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment092(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants092(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate092<T>(items: T[], page: number, size: number): T[] {
