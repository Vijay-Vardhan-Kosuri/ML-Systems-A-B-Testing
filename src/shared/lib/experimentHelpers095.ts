...
export function isActive095(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable095(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart095(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus095(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments095(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc095(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus095(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment095(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants095(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate095<T>(items: T[], page: number, size: number): T[] {
