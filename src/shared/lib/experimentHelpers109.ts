...
export function isActive109(exp: Experiment): boolean { return exp.status==="running"; }
export function isEditable109(exp: Experiment): boolean { return exp.status==="draft"||exp.status==="paused"; }
export function canStart109(exp: Experiment, variants: Variant[]): boolean {
...
export function filterByStatus109(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter(e=>e.status===status);
}
export function searchExperiments109(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc109(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
}
export function countByStatus109(exps: Experiment[]): Record<string, number> {
  const c: Record<string, number>={};
  for (const e of exps) c[e.status]=(c[e.status]||0)+1;
  return c;
}
export function validateExperiment109(exp: Partial<Experiment>): string[] {
  const errors: string[]=[];
...
export function validateVariants109(variants: Variant[]): string[] {
  const errors: string[]=[];
...
export function paginate109<T>(items: T[], page: number, size: number): T[] {
