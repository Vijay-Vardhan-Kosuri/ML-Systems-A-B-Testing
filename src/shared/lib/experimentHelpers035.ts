...
/**
 * Experiment domain helpers 035
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive035(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable035(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart035(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop035(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus035(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments035(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc035(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName035(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus035(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus035(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment035(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants035(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays035(
...
export function assignmentCountByVariant035(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus035(status: ExperimentStatus): string {
...
export function statusColor035(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate035<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment035(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
