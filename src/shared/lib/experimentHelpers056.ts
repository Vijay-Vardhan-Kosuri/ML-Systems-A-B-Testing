...
/**
 * Experiment domain helpers 056
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive056(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable056(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart056(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop056(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus056(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments056(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc056(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName056(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus056(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus056(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment056(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants056(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays056(
...
export function assignmentCountByVariant056(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus056(status: ExperimentStatus): string {
...
export function statusColor056(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate056<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment056(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
