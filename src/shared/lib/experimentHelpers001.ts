...
/**
 * Experiment domain helpers 001
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive001(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable001(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart001(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop001(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus001(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments001(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc001(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName001(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus001(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus001(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment001(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants001(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays001(
...
export function assignmentCountByVariant001(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus001(status: ExperimentStatus): string {
...
export function statusColor001(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate001<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment001(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
