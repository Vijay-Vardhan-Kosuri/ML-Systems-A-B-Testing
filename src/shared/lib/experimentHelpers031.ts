...
/**
 * Experiment domain helpers 031
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive031(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable031(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart031(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop031(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus031(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments031(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc031(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName031(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus031(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus031(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment031(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants031(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays031(
...
export function assignmentCountByVariant031(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus031(status: ExperimentStatus): string {
...
export function statusColor031(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate031<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment031(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
