...
/**
 * Experiment domain helpers 051
 */
import type { Experiment, Variant, ExperimentStatus, Assignment } from "../types";

export function isActive051(exp: Experiment): boolean {
  return exp.status === "running";
}

export function isEditable051(exp: Experiment): boolean {
  return exp.status === "draft" || exp.status === "paused";
}

export function canStart051(exp: Experiment, variants: Variant[]): boolean {
...
export function canStop051(exp: Experiment): boolean {
  return exp.status === "running";
}

export function filterByStatus051(exps: Experiment[], status: ExperimentStatus): Experiment[] {
  return exps.filter((e) => e.status === status);
}

export function searchExperiments051(exps: Experiment[], q: string): Experiment[] {
...
export function sortByCreatedDesc051(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function sortByName051(exps: Experiment[]): Experiment[] {
  return [...exps].sort((a, b) => a.name.localeCompare(b.name));
}

export function groupByStatus051(exps: Experiment[]): Record<string, Experiment[]> {
...
export function countByStatus051(exps: Experiment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const e of exps) counts[e.status] = (counts[e.status] || 0) + 1;
  return counts;
}

export function validateExperiment051(exp: Partial<Experiment>): string[] {
  const errors: string[] = [];
...
export function validateVariants051(variants: Variant[]): string[] {
  const errors: string[] = [];
...
export function estimateDurationDays051(
...
export function assignmentCountByVariant051(assignments: Assignment[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const a of assignments) counts[a.variantId] = (counts[a.variantId] || 0) + 1;
  return counts;
}

export function displayStatus051(status: ExperimentStatus): string {
...
export function statusColor051(status: ExperimentStatus): "default" | "success" | "warning" | "danger" | "info" {
  switch (status) {
...
export function paginate051<T>(items: T[], page: number, pageSize: number): T[] {
  const start = (page - 1) * pageSize;
  return items.slice(start, start + pageSize);
}

export function mergeExperiment051(base: Experiment, patch: Partial<Experiment>): Experiment {
  return {
    ...base,
    ...patch,
