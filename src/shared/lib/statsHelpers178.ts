...
export function uplift178(control: number, treatment: number): number {
  if (control===0) return treatment===0?0:Infinity;
  return (treatment-control)/Math.abs(control);
}
export function relativeUpliftPercent178(c: number, t: number): number { return uplift178(c,t)*100; }
export function isSignificant178(p: number, alpha=0.05): boolean { return p < alpha; }
export function normalCdf178(x: number): number {
  return 0.5*(1+Math.tanh(Math.sqrt(2/Math.PI)*(x+0.044715*x*x*x)));
}
export function pValueFromZ178(z: number): number { return 2*(1-normalCdf178(Math.abs(z))); }
export function twoProportionZ178(cConv: number, cN: number, tConv: number, tN: number): number {
...
export function confidenceInterval178(mean: number, stddev: number, n: number, z=1.96): [number, number] {
  if (n<=1) return [mean, mean];
  const se=stddev/Math.sqrt(n);
  return [mean-z*se, mean+z*se];
}
export function sampleSizeForProportion178(baseline: number, mde: number): number {
...
export function trafficSplitValid178(weights: number[]): boolean {
  if (!weights.length||weights.some(w=>w<0)) return false;
  const s=weights.reduce((a,b)=>a+b,0);
  return Math.abs(s-100)<0.01||Math.abs(s-1)<0.01;
}
export function normalizeWeights178(weights: number[]): number[] {
  const s=weights.reduce((a,b)=>a+b,0);
  return s===0?weights.map(()=>1/weights.length):weights.map(w=>w/s);
}
export function assignVariantHash178(unitId: string, experimentId: string, weights: number[]): number {
  const n=normalizeWeights178(weights);
...
export function summarizeResults178(results: ExperimentResult[]) {
...
export function cohensD178(m1: number, m2: number, pooled: number): number {
  return pooled===0?0:(m2-m1)/pooled;
}
export function pooledStddev178(s1: number, n1: number, s2: number, n2: number): number {
  if (n1+n2<=2) return 0;
